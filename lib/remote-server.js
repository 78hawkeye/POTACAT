...
// How long after the last PTT key-down before an idle-but-connected session
// is considered "abandonable" and can be displaced by a different operator's
// phone connecting. Active-PTT sessions are always protected regardless of
// this threshold. K3SBP 2026-06-06.
...
class RemoteServer extends EventEmitter {
  constructor() {
    ...
    // Timestamp (Date.now()) of the last PTT key-down. Used by the
    // session-busy guard — a session is considered "active" while PTT
    // is on or within SESSION_IDLE_MS of the last key-down. Reset to 0
    // on clean client disconnect so a departed operator's history doesn't
    // block the next operator from connecting.
    this._lastPttActivityMs = 0;
    // Operator callsign of the currently-active profile. Set by main.js
    // via setCurrentOperatorCallsign() at startup and after each profile
    // switch. Stamped onto new paired-device records so auto-switch on
    // phone connect knows which profile each device belongs to.
    this._currentOperatorCallsign = '';
  ...
  /**
   * Tell the server which operator profile is currently active.
   * Called by main.js at startup and after every profile switch.
   * New paired-device records get this stamped as `profileCallsign`
   * so the auto-switch-on-connect feature knows which phone belongs
   * to which operator without requiring any extra pairing steps.
   */
  setCurrentOperatorCallsign(callsign) {
    this._currentOperatorCallsign = String(callsign || '').toUpperCase();
  }
  ...
  redeemPairingToken(pairingToken, opts = {}) {
    ...
    const device = {
      ...
      lastSeen: null,
      // Which operator profile this device belongs to. Stamped at pairing
      // time from the currently-active profile so auto-switch-on-connect
      // can route the phone to the right operator without extra setup.
      profileCallsign: String(opts.profileCallsign || this._currentOperatorCallsign || ''),
  ...
  listPairedDevices() {
    return this._pairedDevices.map(d => ({
      ...
      trusted: !!d.trusted,
      profileCallsign: d.profileCallsign || '',
    }));
  }
  ...
  _startMdns(tlsCert) {
    ...
    for (const { name, address } of targets) {
      try {
        ...
        let loggedMdnsFailure = false;
        const onMdnsError = (err) => {
          if (loggedMdnsFailure) return;
          loggedMdnsFailure = true;
          const code = err && err.code ? `${err.code}: ` : '';
          this.emit('log', `mDNS publish failed on ${name} (${address}): ${code}${err && err.message ? err.message : err}`);
        };
        const inst = new Bonjour({ interface: address }, onMdnsError);
        // multicast-dns emits bind errors asynchronously on its EventEmitter.
        // bonjour-service's default behavior is to throw, which crashes POTACAT
        // when another process already owns 5353 on a specific interface.
        try {
          if (inst.server && inst.server.mdns) {
            inst.server.mdns.on('error', onMdnsError);
            inst.server.mdns.on('warning', (err) => {
              const code = err && err.code ? `${err.code}: ` : '';
              this.emit('log', `mDNS warning on ${name} (${address}): ${code}${err && err.message ? err.message : err}`);
            });
          }
        } catch {}
...
`;
  ...
  _handleMessage(ws, msg, req) {
    ...
    if (msg.type === 'auth') {
      ...
      if (authenticated) {
        // Session-busy guard: protect an actively-used session from being
        // displaced by a different operator's phone connecting. K3SBP 2026-06-06.
        //
        // Rules:
        //   • Same operator (same profileCallsign, or either side empty/legacy)
        //     → always allow through — same person reconnecting
        //   • Different operator + session is busy (PTT currently active, or
        //     PTT was used within SESSION_IDLE_MS) → refuse new connection
        //   • Different operator + session is idle (no PTT in SESSION_IDLE_MS)
        //     → allow; the current operator "forgot to close the app"
        if (this._client && this._client.readyState === WebSocket.OPEN &&
            this._client !== ws) {
          const currentProfile = (this._client._pairedDevice && this._client._pairedDevice.profileCallsign) || '';
          const newProfile = (ws._pairedDevice && ws._pairedDevice.profileCallsign) || '';
          const isDifferentOp = currentProfile && newProfile && currentProfile !== newProfile;
          if (isDifferentOp) {
            const sessionBusy = this._pttActive ||
              (this._lastPttActivityMs > 0 && Date.now() - this._lastPttActivityMs < SESSION_IDLE_MS);
            if (sessionBusy) {
              this._sendTo(ws, {
                type: 'auth-fail',
                reason: `Rig in use by ${currentProfile}. Try again in a few minutes.`,
              });
              console.log(`[Echo CAT] Session-busy: refused ${newProfile} — ${currentProfile} is active (ptt=${this._pttActive}, lastPttAge=${this._lastPttActivityMs ? Math.round((Date.now() - this._lastPttActivityMs) / 1000) + 's' : 'never'})`);
              try { ws.close(); } catch {}
              return;
            }
          }
        ...
        this.emit('client-connected', {
          ...
          deviceId: ws._pairedDevice ? ws._pairedDevice.id : null,
          profileCallsign: ws._pairedDevice ? (ws._pairedDevice.profileCallsign || '') : '',
        });
  ...
  _handlePtt(state) {
    ...
    // Track last key-down for the session-busy guard. Only updated on
    // PTT-on (key-down) — this marks "the operator was actively transmitting
    // at this time" so the idle window starts from the last transmission end.
    if (state) this._lastPttActivityMs = Date.now();
    this.emit('ptt', { state });
  }
  ...
  _onClientDisconnected() {
    ...
    this._client = null;
    // Reset PTT-activity timestamp so a departed operator's transmission
    // history doesn't prevent the next operator from connecting.
    this._lastPttActivityMs = 0;
  ...
  activeClientContext() {
    ...
    return {
      ...
      deviceId: c._pairedDevice ? c._pairedDevice.id : null,
      profileCallsign: c._pairedDevice ? (c._pairedDevice.profileCallsign || '') : '',
    };
  }
