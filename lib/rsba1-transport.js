...
function buildControl(type, seq, sentid, rcvdid) {
  ...
  buf.writeUInt16LE(seq & 0xffff,      6); // mask: counter wraps at 65535
...
class AudioStream extends IcomUdpStream {
  ...
  _sendRxRetransmitRequests() {
    ...
    // Log give-ups at most once per second to avoid flooding the log
    // with individual "no response" lines during large recovery storms.
    if (givenUp.length > 0) {
      const now = Date.now();
      if (!this._giveUpLogLastMs || now - this._giveUpLogLastMs >= 1000) {
        this._log(`[rsba1/${this.name}] no response for ${givenUp.length} missing RX packet(s) seq=${givenUp[0]}…${givenUp[givenUp.length - 1]}; giving up`);
        this._giveUpLogLastMs = now;
      }
    }
    if (!requestSeqs.length) return;
    // Log retransmit requests at most once per second (timer fires every 100ms
    // and each log entry was a synchronous appendFileSync in the caller).
    const now = Date.now();
    const logThis = !this._retxLogLastMs || now - this._retxLogLastMs >= 1000;
    if (logThis) {
      const msg = requestSeqs.length === 1
        ? `[rsba1/${this.name}] requesting missing RX packet seq=${requestSeqs[0]}`
        : `[rsba1/${this.name}] requesting ${requestSeqs.length} missing RX packet(s): ${requestSeqs.join(',')}`;
      this._log(msg);
      this._retxLogLastMs = now;
    }
