...
class CivCodec extends EventEmitter {
  ...
  getDataMod(settingId) {
    ...
    if (!Number.isFinite(id) || id < 0 || id > 0xffff) return;
    this._sendCmd(0x1A, 0x05, [(id >> 8) & 0xff, id & 0xff]);
  }
  ...
  setDataMod(settingId, value) {
    ...
    if (!Number.isFinite(id) || id < 0 || id > 0xffff) return;
    if (!Number.isFinite(v) || v < 0 || v > 0xff) return;
    this._sendCmd(0x1A, 0x05, [(id >> 8) & 0xff, id & 0xff, v & 0xff]);
  }
  ...
  _handleFrame(frame) {
    ...
    const meta = { toAddr: frame[2], fromAddr: frame[3] };
    ...
    // Settings response (cmd 0x1A sub 0x05). Icom uses this command family
    // for menu settings such as MOD Input > DATA1 MOD. The first two bytes
    // after the sub-command are the setting id in the manual's BCD-ish form
    // (0092 => 00 92), followed by the setting value.
    if (cmd === 0x1A && payload.length >= 4 && payload[0] === 0x05) {
      const settingId = ((payload[1] << 8) | payload[2]) & 0xffff;
      const value = payload[3];
      this.emit('data-mod', { settingId, value, meta });
      return;
    }
  }
