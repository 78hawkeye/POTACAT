...
class RigController extends EventEmitter {
  ...
  constructor(model, transport, codec) {
    ...
    this._codec.on('data-mod', (data) => this.emit('data-mod', data));
  ...
  getDataMod(settingId) {
    if (!this.connected) return;
    if (typeof this._codec.getDataMod === 'function') this._codec.getDataMod(settingId);
  }

  setDataMod(settingId, value) {
    if (!this.connected) return;
    if (typeof this._codec.setDataMod === 'function') this._codec.setDataMod(settingId, value);
  }
